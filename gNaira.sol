...
contract gNaira is ERC20, Ownable {
    ...
    // This array contains the addresses of the multisig wallets that are authorized to approve requests
    ...
    function removeApprovedWallet(address wallet) public onlyGovernor {
        ...
        for (uint i = 0; i < _approvedWallets.length; i++) {
            if (_approvedWallets[i] == wallet) {
                _approvedWallets[i] = _approvedWallets[_approvedWallets.length - 1];
                _approvedWallets.pop();
                break;
            }
        }
    }
    ...
    function mintWithApproval(address recipient, uint256 amount) public onlyMinterOrApproved {
    ...
    // Only approved wallets can call this function.
    function approveMint(address _minter, address recipient) public onlyApproved {
        _pendingApprovals[_minter][recipient] = 0;
    }

    // This function approves a burn request for the owner address by burning the specified amount of tokens. 
    // Only approved wallets can call this function.
    function approveBurn(address owner, uint256 amount) public onlyApproved {
        _burn(owner, amount);
    }
    ...
    modifier onlyMinterOrApproved() {
    require(minter[msg.sender] || isApprovedWallet(msg.sender), "Only minter or approved wallets can call this function.");
    _;
}

    modifier onlyApproved() {
    require(isApprovedWallet(msg.sender), "Only approved wallets can call this function.");
    _;
}
